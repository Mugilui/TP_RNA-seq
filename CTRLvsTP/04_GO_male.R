...
dir.create("GO_DEGs/male", recursive = TRUE, showWarnings = FALSE)
...
male_genes <- separate_up_down_genes(deg_male)
...
# Perform GO enrichment for all, up and down-regulated genes
male_all_ego <- perform_go_enrichment(male_genes$all)
male_up_ego <- perform_go_enrichment(male_genes$up)
male_down_ego <- perform_go_enrichment(male_genes$down)
...
if(!is.null(male_up_ego) && !is.null(male_down_ego)) {
  male_dotplot <- custom_dotplot(male_up_ego, male_down_ego, 
                                 "Male DEGs GO enrichment", 
                                 base_size = 20, 
                                 dot_size_range = c(10, 30),
                                 width = 30)
  
  if(!is.null(male_dotplot)) {
    ggsave("GO_DEGs/male/male_custom_dotplot.png", 
           male_dotplot, 
           width = 13,
           height = 13,
           dpi = 300)
...
# Save GO results
save_go_results(male_all_ego, "GO_DEGs/male/male_all_DEGs_GO.csv")
save_go_results(male_up_ego, "GO_DEGs/male/male_up_regulated_GO.csv")
save_go_results(male_down_ego, "GO_DEGs/male/male_down_regulated_GO.csv")

# Print summary of results
cat("\nGO enrichment analysis for male DEGs completed.\n")
cat("Number of all DEGs:", length(male_genes$all), "\n")
cat("Number of up-regulated genes:", length(male_genes$up), "\n")
cat("Number of down-regulated genes:", length(male_genes$down), "\n")
cat("Results saved in GO_DEGs/male directory.\n")